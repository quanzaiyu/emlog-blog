...
$is_reproduct = false;
if(ISLOGIN === true) echo KL_AUTO_BACKUP_AND_MAIL_THE_TIME."\n";
if(KL_AUTO_BACKUP_AND_MAIL_LAST_BACKUP_FILE != '' && file_exists(KL_AUTO_BACKUP_AND_MAIL_LAST_BACKUP_FILE))
{
	...
	if(ISLOGIN === true) echo $delay_time;
}else{
	$is_reproduct = true;
}
if($is_reproduct)
{
	...
	$defname = 'emlog_'. gmdate('Ymd', time() + $timezone * 3600) . '_' . substr(md5(AUTH_KEY . uniqid()), 0, 18);
